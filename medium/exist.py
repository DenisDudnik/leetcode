# https://leetcode.com/problems/word-search/

from collections import Counter
from typing import List


# 2026-03-15
class Solution:
    def exist(self, board: List[List[str]], word: str) -> bool:
        board_letters = Counter(ch for row in board for ch in row)
        word_letters = Counter(word)

        for ch in word_letters:
            if word_letters[ch] > board_letters[ch]:
                return False

        if board_letters[word[0]] > board_letters[word[-1]]:
            word = word[::-1]

        rows, cols = len(board), len(board[0])

        def backtrack(idx: int, row: int, col: int):
            if idx == len(word):
                return True

            if row < 0 or row >= rows or col < 0 or col >= cols or board[row][col] != word[idx]:
                return False

            temp, board[row][col] = board[row][col], "*"
            res = (
                backtrack(idx + 1, row + 1, col)
                or backtrack(idx + 1, row - 1, col)
                or backtrack(idx + 1, row, col + 1)
                or backtrack(idx + 1, row, col - 1)
            )
            board[row][col] = temp
            return res

        for row in range(rows):
            for col in range(cols):
                if board[row][col] == word[0]:
                    if backtrack(0, row, col):
                        return True

        return False


sol = Solution()

board1 = [["A", "B", "C", "E"], ["S", "F", "C", "S"], ["A", "D", "E", "E"]]

assert sol.exist(board1, "ABCCED") is True, "Test case 1 failed"
assert sol.exist(board1, "SEE") is True, "Test case 2 failed"
assert sol.exist(board1, "ABCB") is False, "Test case 3 failed"
